...
export default function PrintLabelsModal({ products, settings, open, onOpenChange }) {
  ...
  React.useEffect(() => {
    if (open) {
      setSelectedProducts([]);
      const initialQuantities = products.reduce((acc, p) => ({ ...acc, [p.id]: (p.stock && p.stock > 0) ? p.stock : 1 }), {});
  ...
  const saveAsPng = async () => {
    if (!printRef.current || labelsToPrint.length === 0) return;
    try {
      const canvas = await html2canvas(printRef.current, {
        scale: 3, // Higher definition
        useCORS: true,
      ...
      link.download = `etiquetas-${new Date().getTime()}.png`;
                ...
                <div key={product.id} className="flex items-center justify-between p-2 border rounded-lg hover:bg-gray-50">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex flex-col items-end">
                      <span className="text-[10px] text-gray-400 font-medium">Estoque: {product.stock || 0}</span>
                      <div className="flex items-center gap-2">
                        <Label htmlFor={`qty-${product.id}`} className="text-xs text-gray-500">Etiquetas:</Label>
                        <Input
                          id={`qty-${product.id}`}
                          type="number"
                          min="1"
